...
### `alphabetize: {order: asc|desc|ignore, caseInsensitive: true|false, caseFirst: lower|upper}`:

Sort the order within each group in alphabetical manner based on **import path**:

- `order`: use `asc` to sort in ascending order, and `desc` to sort in descending order (default: `ignore`).
- `caseInsensitive`: use `true` to ignore case, and `false` to consider case (default: `false`).
- `caseFirst`: use `lower` to sort lowercase imports ahead of uppercase, `upper` for uppercase before lowercase (default: `upper`). This won't have any effect when `caseInsensitive` is `true`.
